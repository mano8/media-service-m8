...
| **Buckets** | `S3_BUCKET_PUBLIC` `S3_BUCKET_PRIVATE` `S3_BUCKET_SENSITIVE` `S3_BUCKET_TEMP` `S3_BUCKET_ARCHIVE` from `media.env` — defaults `public-media` `private-media` `sensitive-media` `temp-media` `archive-media`. All five are copied; `archive-media` is the archive tier (soft-deleted originals awaiting the hard purge, same key as when uploaded), `temp-media` holds transfer exports (`<owner>/exports/<job>.zip`) that expire. |
